...
/* Start Logging Execution */
  ...
  DROP INDEX IF EXISTS NCI_Organisation_AORN ON dbo.Organisation

  DROP INDEX IF EXISTS NCI_Organisation_PAYEScheme ON dbo.OrganisationPAYEScheme

  DROP INDEX IF EXISTS NCI_Organisation_SK ON dbo.Organisation

  DROP INDEX IF EXISTS NCI_Organisation_SK ON dbo.OrganisationAddress

  DROP INDEX IF EXISTS NCI_Organisation_SK ON dbo.OrganisationPAYEScheme

  /* Clear Existing Tables before Load */
  
  DELETE FROM dbo.OrganisationPAYEScheme
  DELETE FROM dbo.OrganisationAddress
  ...
  INSERT INTO [dbo].[Organisation]
   ...
   SELECT @LogID
         ...
         ,(SELECT COUNT(*) FROM dbo.Organisation WHERE RunId=@Run_ID)		                                   


/* Insert Organisation Address Table */

 INSERT INTO [dbo].[OrganisationAddress]
           ([OrgSK]
           ,[TPRUniqueID]
           ,[AddressCode]
           ,[AddressCodeDesc]
  ...
  SELECT Org.OrgSK
        ,Stpr.TPRUniqueID
		,LTRIM(RTRIM(CASE WHEN Stpr.AddressCode='NULL' THEN NULL ELSE Stpr.AddressCode END)) as AddressCode
		,CASE WHEN Stpr.AddressCode='C' THEN 'Correspondence'
              WHEN Stpr.AddressCode='E' THEN 'Employer'
              WHEN Stpr.AddressCode='T' THEN 'Trade'
			  ELSE 'Unknown'
		  END as AddressCodeDesc
		,LTRIM(RTRIM(CASE WHEN Stpr.AddressFromDate='NULL' THEN NULL ELSE CONVERT(VARCHAR,Stpr.AddressFromDate,23) END)) as AddressFromDate
		,LTRIM(RTRIM(CASE WHEN Stpr.AddressToDate='NULL' THEN NULL ELSE CONVERT(VARCHAR,Stpr.AddressToDate,23) END)) as AddressToDate
   ...
   SELECT @LogID
	...
	INSERT INTO [dbo].[OrganisationPAYEScheme]
           ([OrgSK]
           ,[TPRUniqueID]
           ,[PAYEScheme]
           ,[SchemeStartDate]
...
CREATE NONCLUSTERED  INDEX NCI_Organisation_AORN ON dbo.Organisation(AORN)

CREATE NONCLUSTERED  INDEX NCI_Organisation_PAYEScheme ON dbo.OrganisationPAYEScheme(PAYEScheme)

CREATE NONCLUSTERED INDEX NCI_Organisation_SK ON dbo.Organisation(TPRUniqueID,SourceSK)

CREATE NONCLUSTERED  INDEX NCI_Organisation_SK ON dbo.OrganisationAddress(OrgSK,TPRUniqueId,SourceSK)  

CREATE NONCLUSTERED  INDEX NCI_Organisation_SK ON dbo.OrganisationPAYEScheme(OrgSK,TPRUniqueId,SourceSK)  
